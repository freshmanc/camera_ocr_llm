...
不单独做一个「系统管理」程序，而是**在语音助手桌面窗口里加一小块「Web 服务」管理区**，由同一个 main.py 统一管理：

- **入口唯一**：用户只开「语音助手桌面」（main.py），不需要再单独开一个「管理界面」或命令行。
- **状态可见**：在对话窗口顶部或底部固定显示一行，例如：
  - `Web 服务：已停止 | [启动]`  或  `Web 服务：运行中 http://127.0.0.1:8000/ | [停止]`
- **启停操作**：点「启动」→ 在后台用子进程拉起 `uvicorn server.main:app`；点「停止」→ 结束该子进程。关闭语音助手窗口时，若服务是桌面带起来的，一并退出（进程树统一）。
- **访问提示**：运行中时显示「本机访问 http://127.0.0.1:8000/ ；手机同 WiFi 下访问 http://本机IP:8000/」（本机 IP 可自动取或写死在配置里）。

这样：

- **只用桌面**：不点「启动 Web 服务」即可，行为与现在一致。
- **要用 Web/iPhone**：在桌面里点一次「启动」，用浏览器/手机访问上述地址即可；用完后点「停止」或直接关桌面，服务一起停。
